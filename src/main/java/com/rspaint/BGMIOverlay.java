...
//			if (sub.x > 1.05 || sub.x < -0.05 || sub.y > 1.05 || sub.y < -0.05) System.out.println("here " + sub.x + " " + sub.y);
	...
	List<ToolbarButton> buttons = new ArrayList<>();
	{
		buttons.add(new ToolbarButton() {
			...
			@Override public void addMenuEntries() {
				client.createMenuEntry(-1).setTarget("New color").setOption("").onClick(me -> pickColor(-1));
			}
		});
		for (int i = 0; i < 10; i++) {
	...
	class ColorToolbarButton extends ToolbarButton {
		...
		@Override public void addMenuEntries() {
			client.createMenuEntry(1).setOption(ColorUtil.wrapWithColorTag("Choose", colors.get(colorIndex))).onClick(me -> {
...
//
	...
	//	void saveStrokes() {
//		// Put the strokes together by their primary region.
//		Multimap<Integer, Stroke> regionIdToStroke = ArrayListMultimap.create();
//		// This is for regions that must also be loaded when a region is loaded, because those other regions contain multi-region strokes that have points in this region.
//		Map<Integer, Set<Integer>> bonusRegionIds = new HashMap<>();
//		for (Stroke stroke : strokes) {
//			regionIdToStroke.put(stroke.primaryRegion(), stroke);
//			for (Integer regionId : stroke.regions) {
//				bonusRegionIds.computeIfAbsent(regionId, k -> new HashSet<>()).add(stroke.primaryRegion());
//			}
//		}
//		for (Map.Entry<Integer, Collection<Stroke>> entry : regionIdToStroke.asMap().entrySet()) {
//			Integer regionId = entry.getKey();
//			configManager.setConfiguration("rspaint", "strokes_" + regionId, gson.toJson(new RegionStrokes((List<Stroke>) entry.getValue(), bonusRegionIds.get(regionId))));
//		}
//	}
//
	...
	//					sy = ((int) (p.getSuby() * 128f)) / 128f;
					...
					if (lastx != -1)
					{
						if (erasing)
						{
//							if (aabbCheck(pf(lastx, lasty), pf(screenx, screeny), pf(client.getMouseCanvasPosition()), pf(lastMouseCanvasPosition), 10))
//							{
//								// https://stackoverflow.com/questions/563198/how-do-you-detect-where-two-line-segments-intersect
//								pf lastMouse = pf(lastMouseCanvasPosition);
//								pf mouse = pf(client.getMouseCanvasPosition());
//								pf lastScreen = pf(lastx, lasty);
//								pf screen = pf(screenx, screeny);
//								pf r = sub(lastMouse, mouse);
//								pf s = sub(lastScreen, screen);
//								float rxs = cross(r, s);
//								if (rxs != 0) { // not parallel
//
//								}
//								closestDistance(lastScreen, screen, mouse);
//								closestDistance(lastScreen, screen, lastMouse);
//								closestDistance(lastMouse, mouse, lastScreen);
//								closestDistance(lastMouse, mouse, screen);
...
//						graphics.drawRect(screenx - 1, screeny - 1, 2, 2);
//						graphics.drawRect(lastx - 1, lasty - 1, 2, 2);
//						graphics.setColor(s.color);
					}
					lastx = screenx;
					lasty = screeny;
...
//		System.out.println(count);
	...
	private boolean aabbCheck(pf pa1, pf pa2, pf pb1, pf pb2, int min)
	{
		boolean a1Largerx = pa1.x > pa2.x;
		float aminx = (a1Largerx ? pa1.x : pa2.x) - min;
		if (pb1.x < aminx && pb2.x < aminx) return false;
		float amaxx = (a1Largerx ? pa2.x : pa1.x) + min;
		if (pb1.x > amaxx && pb2.x > amaxx) return false;

		boolean a1Largery = pa1.y > pa2.y;
		float aminy = (a1Largery ? pa1.y : pa2.y) - min;
		if (pb1.y < aminy && pb2.y < aminy) return false;
		float amaxy = (a1Largery ? pa2.y : pa1.y) + min;
		if (pb1.y > amaxy && pb2.y > amaxy) return false;

		return true;
	}

//	public pf getCanvasLocation(
//		int x,
//		int y,
//		int subx,
//		int suby,
...
//		int zOffset)
//	{
//		final WorldView wv = client.getWorldView();
//		final int msx = x - wv.getBaseX() - ESCENE_OFFSET;
//		final int msy = y - wv.getBaseY() - ESCENE_OFFSET;
//
...
//		}
//
//		final float swX = x - subx * 128;
//		final float swY = y - suby * 128;
//
//		final float neX = x + subx * 128;
//		final float neY = y + subx * 128;
//
//		final float seX = swX;
//		final float seY = neY;
//
...
//		if (e.getGroup().equals("rspaint") && e.getKey().equals("drawColor")) {
//			colors.add(0, config.drawColor());
//			saveColors();
//		}
	}
